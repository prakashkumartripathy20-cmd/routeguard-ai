...
// Replace from window.onload = function() down to checkQuerySpot() end
const badBlockRegex = /window\.onload = function\(\)[\s\S]*?checkQuerySpot\(\);\s*}/;
if (badBlockRegex.test(serverContent)) {
  serverContent = serverContent.replace(badBlockRegex, cleanLocateCode);
} else {
  // Replace window.onload = initMaps;
  serverContent = serverContent.replace('window.onload = initMaps;', cleanLocateCode);
}

fs.writeFileSync(path.join(__dirname, '..', 'server.js'), serverContent);
console.log("Cleaned server.js completely!");
